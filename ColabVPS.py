...
class _NoteProgress(apt.progress.base.InstallProgress, apt.progress.base.AcquireProgress, apt.progress.base.OpProgress):
  def __init__(self):
    ...
    display(self._label)
    self._float_progress = ipywidgets.FloatProgress(min = 0.0, max = 1.0, layout = {'border':'1px solid #000e88'})
...
def _check_gpu_available():
  ...
  return IPython.utils.io.ask_yes_no("Do you want to continue? (don't press no, do the things that I said above instead) [y/n]")
...
def _setupSSHDMain(public_key, tunnel, ngrok_region, check_gpu_available, mount_gdrive_to, mount_gdrive_from, is_VNC):
  ...
  print("---")
  if tunnel == None:
    print("ngrok good argo bad use ngrok instead")
    print("                     -Hien-")
    tunnel = "ngrok"
...
def setupVNC(ngrok_region = ap, check_gpu_available = True, tunnel = None, mount_gdrive_to = None, mount_gdrive_from = None, public_key = None):
  stat, msg = _setupSSHDMain(public_key, tunnel, check_gpu_available, mount_gdrive_to, mount_gdrive_from, True)
  if stat:
    msg += _setupVNC()
